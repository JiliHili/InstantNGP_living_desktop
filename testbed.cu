...
void Testbed::init_window(int resw, int resh, bool hidden, bool second_window) {
	...
	glfwWindowHint(GLFW_MAXIMIZED | GLFW_VISIBLE, hidden ? GLFW_FALSE : GLFW_TRUE);
	std::string title = "Changing glfwapi";
	m_glfw_window = glfwCreateWindow(m_window_res.x, m_window_res.y, title.c_str(), NULL, NULL);
	/*The part of live wallpaper code part(window)*/
	HWND render_window = glfwGetWin32Window(m_glfw_window);
	//HWND progman = FindWindow("Progman", NULL);
	if (!TRUE) {
		glfwTerminate();
		printf("Progman not found,live wallpaper're fxxking lost,\n");
		return void();
	}
	//SendMessageTimeout(progman, 0x052C, 0, 0, SMTO_NORMAL, 1000, NULL);
	/*
	EnumWindows([](HWND i_window, LPARAM lParam) -> BOOL {
		printf("searching for WorkerW\n");
		HWND shell = FindWindowEx(i_window, NULL, "SHELLDLL_DefView", NULL);
		if (shell != NULL) {
			HWND worker = FindWindowEx(NULL, i_window, "WorkerW", NULL);
			printf("shell found");
			if (worker != NULL) {
				*(HWND*)lParam = i_window;
				printf("worker returns to desktop.hwnd");
				return FALSE;
			}
		}
		return TRUE;
	...
	*/
	//if (desktop == NULL) {
	//	desktop = FindWindowEx(NULL, progman, "WorkerW", NULL);
	//}


	//SetParent(desktop, progman);
	// Set the GLFW window as a child of the desktop
	/*Same codepart but its for windows-11*/
	/*/Same codepart but its for windows-11*/
	/*
	LONG style = GetWindowLong(render_window, GWL_STYLE);
	style &= ~(WS_CAPTION | WS_THICKFRAME | WS_MINIMIZE | WS_MAXIMIZE | WS_SYSMENU);
	SetWindowLong(render_window, GWL_STYLE, style);

	// Set the window to be layered and transparent (optional, depending on your needs)
	SetWindowLong(render_window, GWL_EXSTYLE, GetWindowLong(render_window, GWL_EXSTYLE) | WS_EX_LAYERED);
	SetLayeredWindowAttributes(render_window, 0, 255, LWA_ALPHA); // Fully opaque

	// Position the window (e.g., full desktop size)
	SetWindowPos(render_window, HWND_BOTTOM, 0, 0, 2560, 1440, SWP_SHOWWINDOW);
	...
	*/
	...
	if (hDefView!=NULL) {
		...
		if (hWorkerW != NULL) {
			...
			POINT ptPos = { 0, 0 };
			SIZE sizeWnd = { 256, 256 };
			POINT ptSrc = { 0, 0 };
			HDC hdcScreen = GetDC(NULL);
			HDC hDC = CreateCompatibleDC(hdcScreen);
			SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_SYSTEM_AWARE);
			int hWnd_width = GetDeviceCaps(hdcScreen, HORZRES);
			int hWnd_height = GetDeviceCaps(hdcScreen, VERTRES);
			DWORD style = WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS | WS_MAXIMIZE;
			DWORD styleEx = WS_EX_TOOLWINDOW | WS_EX_LAYERED;
			RECT rc = { 0, 0, 800, 600 };
			AdjustWindowRectEx(&rc, style, FALSE, styleEx);
			LONG Cstyle = GetWindowLong(render_window, GWL_STYLE);
			Cstyle |= WS_CHILDWINDOW | WS_MAXIMIZEBOX | WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS;
			Cstyle &= ~(WS_CAPTION | WS_THICKFRAME | WS_MINIMIZE | WS_OVERLAPPED | WS_THICKFRAME | WS_CLIPCHILDREN);
			SetWindowLong(render_window, GWL_STYLE, Cstyle);
			LONG CstyleEx = GetWindowLong(render_window, GWL_EXSTYLE);
			CstyleEx |= WS_EX_WINDOWEDGE;
			CstyleEx &= ~(WS_EX_ACCEPTFILES);
			CstyleEx |= WS_EX_LAYERED;
			SetWindowLong(render_window, GWL_EXSTYLE, CstyleEx);
			glfwMaximizeWindow(m_glfw_window);
			ShowWindow(render_window, SW_MAXIMIZE);
			UpdateLayeredWindow(render_window, hdcScreen, &ptPos, &sizeWnd, hDC, &ptSrc, 0, &blend, ULW_ALPHA);
			ShowWindow(render_window, SW_MAXIMIZE);
			SetLayeredWindowAttributes(render_window, RGB(0, 0, 0), 255, LWA_ALPHA);
			UpdateWindow(render_window);
			ShowWindow(render_window, SW_MAXIMIZE);
			SetParent(render_window, hWorkerW);
			SetWindowPos(render_window, 0, 0, 0, hWnd_width, hWnd_height, SWP_SHOWWINDOW);
			//SetParent(render_window, hWorkerW);
		}
	}
